...
// PLACEHOLDERS TEMPORALES IDENTIFICADOS CLARAMENTE COMO DATOS DEMO
const academicMetricsDemo: AcademicMetric[] = [
  { label: 'Certificados emitidos', value: 1248, trend: '+8.4% este mes', trendType: 'positive' },
  { label: 'Homologaciones aprobadas', value: 342, trend: '+12.1% este mes', trendType: 'positive' },
  { label: 'Solicitudes finalizadas', value: 1894, trend: '+6.2% este mes', trendType: 'positive' },
  { label: 'Cancelaciones', value: 114, trend: '-2.4% este mes', trendType: 'neutral' },
]

const operationalMetricsDemo: OperationalMetric[] = [
  { label: 'Tiempo prom. respuesta', value: '1.8 días', trend: '-0.5 días vs mes ant.', trendType: 'positive' },
  { label: 'Procesadas esta semana', value: 47, trend: '+14.6% vs sem. ant.', trendType: 'positive' },
  { label: 'Casos vencidos', value: 2, trend: '-33% vs sem. ant.', trendType: 'positive' },
  { label: 'SLA', value: '98.2%', trend: '+1.2% de efectividad', trendType: 'positive' },
]

const academicAlertsDemo: AcademicAlert[] = [
  { message: '3 solicitudes próximas a vencer', detail: 'Vencimiento en menos de 24 horas', severity: 'danger' },
  { message: '2 homologaciones pendientes > 5 días', detail: 'Supera el tiempo estimado de atención', severity: 'warning' },
  { message: '1 certificado requiere revisión', detail: 'Documento oficial generado pendiente de firma', severity: 'info' },
  { message: '4 documentos faltantes', detail: 'Solicitudes en espera de subsanación por alumno', severity: 'warning' },
]
...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-6 lg:space-y-8 relative overflow-hidden">
      {/* Background Meshes for premium backdrop depth */}
      ...
      <div className="relative overflow-hidden rounded-2xl glass-panel p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-premium-sm border-l-4 border-l-[#e8a820] z-10">
        ...
        <div className="absolute right-0 top-0 opacity-[0.06] dark:opacity-[0.02] transform translate-x-12 -translate-y-8 pointer-events-none">
        ...
        <div className="space-y-3 max-w-4xl flex-1 z-10">
          ...
          <h2 className="font-display font-extrabold text-2xl md:text-3xl text-[#0d1b3e] dark:text-white tracking-tight leading-tight">
          ...
          <div className="pt-4 flex max-w-xl w-full relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 text-sm rounded-xl border border-navy-200 dark:border-slate-800 bg-white text-slate-950 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#e8a820]/45 focus:border-[#e8a820] transition-all placeholder-slate-500 dark:placeholder-slate-400 shadow-sm"
            />
          </div>
        </div>
          ...
          <div className="glass-card rounded-2xl p-5 relative overflow-hidden group">
            <div className="flex justify-between items-start">
              <div className="space-y-1">
                ...
                <h3 className="text-3xl font-display font-extrabold text-[#0d1b3e] dark:text-white tracking-tight">
                  {total}
                </h3>
              </div>
              <div className="p-2.5 rounded-xl bg-slate-100 border border-slate-200 text-[#0d1b3e] dark:bg-slate-800 dark:border-transparent dark:text-slate-350">
            ...
            <div className="mt-4 flex items-center justify-between gap-4">
              ...
              {/* SVG Sparkline */}
              <svg className="w-20 h-7 text-emerald-650 dark:text-emerald-500" viewBox="0 0 100 30" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <div className="space-y-6 xl:col-span-8">
            ...
            <section className="glass-panel rounded-2xl shadow-premium-sm p-5 space-y-4">
              <div className="flex items-center justify-between gap-4 flex-wrap">
                <div>
                  <h3 className="font-display font-bold text-base text-[#0d1b3e] dark:text-white">
                ...
                <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-250 dark:border-slate-700/20 shrink-0">
                  <button
                    ...
                    className={cn(
                      ...
                      feedFilter === 'all'
                        ...
                        : "text-slate-750 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                    )}
                  >
                    Todos
                  </button>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/60 -mx-3">
                {filteredActivity.length === 0 ? (
                  <div className="py-8 flex flex-col items-center justify-center text-center">
                    <AlertCircle className="w-8 h-8 text-slate-500 dark:text-slate-500 mb-2" />
                    <h4 className="text-xs font-bold text-slate-900 dark:text-slate-300">No se encontraron solicitudes</h4>
                    <p className="text-[11px] text-slate-800 dark:text-slate-400 max-w-[200px] mt-1">Prueba a buscar con otra palabra o cambia el filtro.</p>
                  </div>
                ) : (
                        ...
                        <div className="flex items-center gap-3.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="px-2 py-0.5 text-[9px] font-bold bg-slate-100 border border-slate-250 text-slate-800 dark:bg-slate-800 dark:text-slate-350 rounded-md uppercase tracking-wider shrink-0 dark:border-slate-700/50">
            ...
            <section className="glass-panel rounded-2xl shadow-premium-sm p-5 space-y-4">
              <div>
                <div className="flex items-center gap-2 justify-between">
                  ...
                  <Badge variant="outline" className="text-[10px] py-0 px-1.5 font-bold uppercase border-yellow-500/30 text-yellow-600 bg-yellow-500/10 dark:text-yellow-400 dark:bg-yellow-400/10">
                    Demo
                  </Badge>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {academicMetricsDemo.map((metric, index) => (
                  <div key={index} className="rounded-xl border border-slate-300 dark:border-slate-800/40 bg-white dark:bg-slate-900/35 p-4 transition-all hover:bg-slate-100/30 dark:hover:bg-slate-850/60 hover:shadow-sm">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={cn(
                        ...
                        metric.trendType === 'positive' && "bg-emerald-50 text-emerald-700 border-emerald-250 dark:bg-success-soft dark:text-success dark:border-success/20",
                        metric.trendType === 'neutral' && "bg-amber-50 text-amber-800 border-amber-200 dark:bg-warning-soft dark:text-warning dark:border-warning/20",
                        metric.trendType === 'negative' && "bg-red-50 text-red-700 border-red-200 dark:bg-danger-soft dark:text-danger dark:border-danger/20",
                      )}>
                        {metric.trendType === 'positive' && <TrendingUp className="w-3.5 h-3.5 inline" />}
                        {metric.trend}
                      </span>
                    </div>
                    <p className="mt-2 text-3xl font-black font-display text-[#0d1b3e] dark:text-white tracking-tight">
                      {metric.value}
                    </p>
                  </div>
                ))}
            ...
            <section className="glass-panel rounded-2xl shadow-premium-sm p-5 space-y-4">
              ...
              <div className="space-y-4">
                <div className="h-5 rounded-2xl bg-slate-200/50 dark:bg-slate-800 flex overflow-hidden border border-slate-250 dark:border-slate-700/20">
                  {/* Aprobadas */}
                  {approvedPct > 0 && (
                    <div
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 pt-4">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <h5 className="text-xs font-bold text-slate-700 dark:text-slate-200">Aprobadas</h5>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{approved} {approved === 1 ? 'solicitud' : 'solicitudes'} ({Math.round(approvedPct)}%)</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <h5 className="text-xs font-bold text-slate-700 dark:text-slate-200">Borradores</h5>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{draft} {draft === 1 ? 'solicitud' : 'solicitudes'} ({Math.round(draftPct)}%)</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <h5 className="text-xs font-bold text-slate-700 dark:text-slate-200">En revisión</h5>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{inReview} {inReview === 1 ? 'solicitud' : 'solicitudes'} ({Math.round(inReviewPct)}%)</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <h5 className="text-xs font-bold text-slate-700 dark:text-slate-200">Enviadas</h5>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{submitted} {submitted === 1 ? 'solicitud' : 'solicitudes'} ({Math.round(submittedPct)}%)</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <h5 className="text-xs font-bold text-slate-700 dark:text-slate-200">Rechazadas</h5>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{rejected} {rejected === 1 ? 'solicitud' : 'solicitudes'} ({Math.round(rejectedPct)}%)</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <h5 className="text-xs font-bold text-slate-700 dark:text-slate-200">Canceladas</h5>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{cancelled} {cancelled === 1 ? 'solicitud' : 'solicitudes'} ({Math.round(cancelledPct)}%)</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6 xl:col-span-4 xl:sticky xl:top-6 xl:self-start">
              ...
              <div className="space-y-2.5">
                ...
                <div className={cn(
                  ...
                  inReview > 0
                    ...
                    : "bg-slate-50/40 dark:bg-slate-900/15 border-slate-200/80 dark:border-slate-800/40 text-slate-600 dark:text-slate-450"
                )}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">Trámites siendo evaluados</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className={cn(
                  "flex items-center justify-between p-3.5 rounded-xl border transition-all hover-elevate",
                  submitted > 0
                    ? "bg-amber-50/60 dark:bg-amber-950/20 border-amber-250 dark:border-amber-900/50 shadow-sm text-amber-950 dark:text-amber-100"
                    : "bg-slate-50/40 dark:bg-slate-900/15 border-slate-200/80 dark:border-slate-800/40 text-slate-600 dark:text-slate-450"
                )}>
                  <div className="flex items-center gap-3">
                    <div className={cn(
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">Nuevas solicitudes por asignar</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className={cn(
                  "flex items-center justify-between p-3.5 rounded-xl border transition-all hover-elevate",
                  draft > 0
                    ? "bg-slate-100/80 dark:bg-slate-850 border-slate-350 dark:border-slate-800 shadow-sm text-slate-950 dark:text-slate-100"
                    : "bg-slate-50/40 dark:bg-slate-900/15 border-slate-200/80 dark:border-slate-800/40 text-slate-600 dark:text-slate-450"
                )}>
                  <div className="flex items-center gap-3">
                    <div className={cn(
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">Guardadas sin enviar por el alumno</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {draft > 0 && (
                      <Link to="/dashboard/requests?status=DRAFT" className="p-1 text-slate-600 hover:text-slate-850 dark:text-slate-400 dark:hover:text-white transition-colors">
                        <ArrowRight className="w-4 h-4" />
                      </Link>
                    )}
                  </div>
                ...
                {quickLinks.map((link) => {
                  ...
                  return (
                    <Link
                      key={link.to}
                      to={link.to}
                      className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-50/60 dark:bg-slate-900/40 border border-slate-250 dark:border-slate-850 hover:bg-white hover:shadow-sm hover:border-[#e8a820] text-slate-850 dark:text-slate-350 hover:text-[#0d1b3e] dark:hover:text-white transition-all group hover-elevate border"
                    >
            ...
            <section className="glass-panel rounded-2xl shadow-premium-sm p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                {operationalMetricsDemo.map((metric, index) => (
                  <div key={index} className="rounded-xl border border-slate-300 dark:border-slate-800/40 bg-white dark:bg-slate-900/35 p-3 hover:bg-slate-100/30 dark:hover:bg-slate-850/60 transition-colors">
                    <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block leading-tight">
                      {metric.label}
                    </span>
                    <p className="mt-1.5 text-lg font-bold font-display text-[#0d1b3e] dark:text-white leading-none">
                      {metric.value}
                    </p>
                    <span className={cn(
                      "text-[9px] font-bold block mt-1",
                      metric.trendType === 'positive' && "text-emerald-700 dark:text-emerald-400",
                      metric.trendType === 'neutral' && "text-[#b45309] dark:text-amber-400",
                      metric.trendType === 'negative' && "text-red-700 dark:text-red-400",
                    )}>
                      {metric.trend}
                    </span>
                  </div>
            ...
            <section className="glass-panel rounded-2xl shadow-premium-sm p-5 space-y-4">
              ...
              <div className="space-y-2.5">
                {academicAlertsDemo.map((alert, index) => (
                  <div key={index} className="flex items-start gap-2.5 p-2.5 rounded-xl border border-slate-300 dark:border-slate-800/40 bg-white dark:bg-slate-900/35 hover:bg-slate-100/30 dark:hover:bg-slate-850/60 transition-colors">
                    <div className={cn(
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[9px] text-slate-500 dark:text-slate-400 truncate mt-0.5">{alert.detail}</p>
                    </div>
                  </div>
                ))}
              </div>
